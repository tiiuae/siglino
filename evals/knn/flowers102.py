...
def main():
    ...
    # Data paths
    parser.add_argument("--train_images_root", type=str, required=True, help="Path to training images (one folder per class)")
    parser.add_argument("--val_images_root", type=str, required=True, help="Path to validation/test images (one folder per class)")
    ...
    if (not using_distributed) or (rank == 0):
        ...
        with open(os.path.join(args.output_dir, f"flowers102_knn.json"), "w") as f:
